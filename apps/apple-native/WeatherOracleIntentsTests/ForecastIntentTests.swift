import XCTest
import AppIntents
@testable import WeatherOracleIntents
...
final class ForecastIntentTests: XCTestCase {
    ...
    func testQuickActionsAvailable() {
        let quickActions = WeatherOracleShortcuts.quickActions

        // Should have at least 2 quick actions
        XCTAssertGreaterThanOrEqual(quickActions.count, 2)
    }

    // MARK: - Shortcut Groups Tests

    @available(iOS 16.0, macOS 13.0, *)
    func testBasicWeatherShortcutGroup() {
        let basicWeather = ShortcutGroups.basicWeather

        XCTAssertFalse(basicWeather.isEmpty)
        XCTAssertTrue(basicWeather.first?.intent is ForecastIntent)
    }

    @available(iOS 16.0, macOS 13.0, *)
    func testModelComparisonShortcutGroup() {
        let modelComparison = ShortcutGroups.modelComparison

        // Should have 3 comparison shortcuts (temp, precip, wind)
        XCTAssertEqual(modelComparison.count, 3)

        // All should be CompareIntent
        for shortcut in modelComparison {
            XCTAssertTrue(shortcut.intent is CompareIntent)
        }
    }
